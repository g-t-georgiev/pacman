...
export const GHOSTS_IDS = {
  ...
  1: GhostAliases.Cyan,
  2: GhostAliases.Pink,
  3: GhostAliases.Orange
};

/**
 * @param {number} tileSize 
 * @returns {Record<GhostAliases, import("../constants.js").ActorData>}
 */
export const getGhostsData = (tileSize) => {
  return Object.values(GhostAliases).reduce((obj, name) => {
    const rawData = ACTORS_DATA[name];

    obj[name] = {
      ...rawData,
      position: {
        x: rawData.position.x * tileSize,
        y: rawData.position.y * tileSize,
      },
    }
